...
export default function Moods() {
  ...
  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: (mood==='Happy' ? 'royalblue' : 'crimson'),
  };

  return (
    <div className='widget-moods container'>
      <h2>Moods</h2>
      <div id='mood' style={style}>{mood==='Happy'?happyMood : mood==='Sad' ? sadMood : initialMood}</div> {/* STEP 3 */}
      <div>
        <button id='makeHappy' onClick={makeHappy}>Happy Mood</button>
        <button id='makeSad' onClick={makeSad}>Sad Mood</button>
